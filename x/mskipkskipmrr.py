...
def kskipmrr(A, b, epsilon, k, T, pu):
    ...
    # 初期反復
    start_time = start(method_name='m-skip k-skip MrR', k=k)
    Ar[1] = dot(A, Ar[0])
    ...
    while i < max_iter:
        ...
        if i % (2 * (k + 1)): # rskip
            ...
            alpha[0] = dot(Ar[0], Ar[0])
            alpha[1] = dot(Ar[0], Ar[1])
            alpha[2] = dot(Ar[1], Ar[1])
            beta[1] = dot(Ay[0], Ar[1])
            delta[0] = dot(Ay[0], Ay[0])
